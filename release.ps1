...
exit /b
#>
# release.ps1 - auto-increment the v1.0.x tag and push it to trigger the iOS
# tweak CI build (.github/workflows/build.yml). The CI builds MonitorTweak.dylib
# on a macOS runner via Theos and publishes MonitorTweak.zip as a GitHub Release.
# Commits and pushes any pending main changes first.
...
Write-Host ""
Write-Host "Done. CI will build MonitorTweak at ref=$nextTag and publish MonitorTweak.zip as a GitHub Release."
Write-Host "After it finishes: download MonitorTweak.zip, extract MonitorTweak.dylib, and point ios.tweak_dylib_path at it (do not rename/re-zip — the zip preserves the build timestamp)."
